...
We assessed our GLS model type I error rates (proportion of $p$-values below 0.05) and calibration using a null model of random traits and genotype data from the 1000 Genomes Project Phase III.
...
We ran all the standard procedures for the TWAS approaches (S-PrediXcan and S-MultiXcan), including: 1) a standard GWAS using linear regression under an additive genetic model, 2) various GWAS processing steps, including harmonization and imputation procedures as defined in [@doi:10.1002/gepi.22346], 3) S-PrediXcan and S-MultiXcan analyses.
Below, we provide details for each of these steps.

**Step 1 - GWAS**.
We performed standard QC procedures, filtering out variants with missing call rates exceeding 0.01, MAF below 1%, or MAC below 20, and HWE below 1e-6.
...
This step includes the harmonization of GWAS data and the imputation of $z$-scores, which are integral components of the TWAS pipeline.
These processes are necessary to ensure sufficient overlap with SNPs in the prediction models.
The scripts to execute these steps can be found at [@url:https://github.com/hakyimlab/summary-gwas-imputation].
These procedures were applied to all 1,000 random phenotypes, resulting in a total of 8,325,729 variants, encompassing both original and imputed $z$-scores.
...
Importantly, the GLS model is able to correct for LVs with adjacent and highly correlated genes at the top such as LV234 (Figure @fig:reg:nulls:qqplot:lv234), LV847 (Figure @fig:reg:nulls:qqplot:lv847), LV45 (Figure @fig:reg:nulls:qqplot:lv45), and LV800 (Figure @fig:reg:nulls:qqplot:lv800), among others.
...
We also detected other latent variables (LVs) with higher-than-expected mean type I errors for both the generalized least squares (GLS) and ordinary least squares (OLS) models, although these LVs do not have a relatively large number of adjacent genes at the top.
One example is LV914, shown in Figure @fig:reg:nulls:qqplot:lv914.
The inflation in these LVs might be explained by inaccuracies in correlation estimates between the individual-level MultiXcan model and its summary-based version (see Methods).
Therefore, we flagged those LVs with a type I error rate larger than 0.07 (127 LVs in total) and excluded them from our main analyses.
We did not observe signs of inflation when applying the method to real data (Figure @fig:reg:real:qqplots).
...
For the second scenario, we projected matrix $\mathbf{M}$ into the latent space, and then shuffled the latent variables (LVs) in $\hat{\mathbf{M}}$ for each trait.
...
The proofread paragraph is as follows:
...
This indicates, as expected, that there is no consensus among ensemble partitions generated with different clustering algorithms and data representations.
In contrast, when using real data, the consensus clustering approach identifies trait pairs that are consistently grouped together across the different members of the ensemble.
The partitions above the 75th percentile were considered in the main analyses and are depicted in the clustering tree in Figure @fig:clustering:tree.
